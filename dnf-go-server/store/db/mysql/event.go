...
func (d *DB) UpsertEventProgress(ctx context.Context, p *store.EventProgress) (*store.EventProgress, error) {
	...
	_, err := d.db.ExecContext(ctx, `
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE progress_value = VALUES(progress_value), status = VALUES(status), updated_at = VALUES(updated_at)
   `, now, now, store.RowStatusNormal, p.RoleID, p.EventID, p.ProgressType, p.ProgressValue, p.Status)
	if err != nil {
		return nil, fmt.Errorf("failed to upsert event progress: %w", err)
	}
	return p, nil
}
