...
func (d *DB) SearchPartyList(ctx context.Context, dungeonIndex, minLevel, maxLevel uint32) ([]*store.PartyInfo, error) {
	...
	if dungeonIndex > 0 {
		query += fmt.Sprintf(" AND p.dungeon_index = ?")
		args = append(args, dungeonIndex)
		argIndex++
	}

	if minLevel > 0 {
		query += fmt.Sprintf(" AND p.min_level >= ?")
		args = append(args, minLevel)
		argIndex++
	}

	if maxLevel > 0 {
		query += fmt.Sprintf(" AND p.max_level <= ?")
...
func (d *DB) HalfOpenPartyAccept(ctx context.Context, roleID, partyGuid, targetGuid uint64) error {
	...
	for _, rid := range applicants {
		...
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO t_party_member (party_id, role_id, player_id, team_type, status, join_time) VALUES (?, ?, ?, 0, 0, NOW())`,
...
func (d *DB) HalfOpenPartyJoin(ctx context.Context, roleID, partyGuid uint64) error {
	...
	case 1:
		...
		if _, err := d.db.ExecContext(ctx,
			"INSERT IGNORE INTO t_party_request (party_id, role_id, create_time) VALUES (?, ?, NOW())",
...
func (d *DB) TeamRankPosition(ctx context.Context, roleID uint64) (rank, total int, err error) {
	...
	for i, t := range teams {
		if t.partyID == myParty.PartyGuid {
