...
func (d *DB) SubmitPvpBattleResult(ctx context.Context, record *store.PvpRecord, stats *store.PvpStats) error {
	...
	if _, err := tx.ExecContext(ctx,
		"INSERT INTO t_pvp_record (created_at, updated_at, row_status, role_id, match_type, win, score, opponent_id, battle_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
		now, now, store.RowStatusNormal, record.RoleID, record.MatchType, record.Win, record.Score, record.OpponentID, record.BattleTime); err != nil {
	...
	case errors.Is(err, sql.ErrNoRows):
		if _, err := tx.ExecContext(ctx,
			"INSERT INTO t_pvp_stats (created_at, updated_at, row_status, role_id, total_matches, win_count, lose_count, total_score, max_win_streak, current_streak) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
			now, now, store.RowStatusNormal, stats.RoleID, stats.TotalMatches, stats.WinCount, stats.LoseCount,
...
func (d *DB) UpsertRaidEntrance(ctx context.Context, e *store.RaidEntrance) error {
	now := time.Now().Unix()
	query := "INSERT INTO t_raid_entrance (created_at, updated_at, row_status, role_id, raid_index, daily_character_count, character_count, account_count, daily_reward_count, reward_count, last_enter_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(role_id, raid_index) DO UPDATE SET updated_at = excluded.updated_at, daily_character_count = excluded.daily_character_count, character_count = excluded.character_count, account_count = excluded.account_count, daily_reward_count = excluded.daily_reward_count, reward_count = excluded.reward_count, last_enter_time = excluded.last_enter_time"
	if _, err := d.db.ExecContext(ctx, query,
