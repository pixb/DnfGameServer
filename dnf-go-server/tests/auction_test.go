...
// loginWithOpenid 登录并选择角色(openid 不存在时自动创建角色)
func (s *AuctionTestSuite) loginWithOpenid(openid, charName string) uint64 {
	// 1. 登录
...
// loginAsSeller 卖家 001 登录并返回角色 uid
func (s *AuctionTestSuite) loginAsSeller() uint64 {
	return s.loginWithOpenid("test_user_auction_001", "AuctionHero")
}

// loginAsBuyer 买家 002 登录并返回角色 uid(附带充足金币)
func (s *AuctionTestSuite) loginAsBuyer() uint64 {
	charguid := s.loginWithOpenid("test_user_auction_002", "AuctionHero2")
	if charguid > 0 {
		if err := setGold(charguid, 100000); err != nil {
			s.T().Logf("set gold for buyer failed: %v", err)
		}
	}
	return charguid
}

// registerAuctionAsSeller 卖家 001 发布一件拍卖,返回 auctionId
func (s *AuctionTestSuite) registerAuctionAsSeller() uint64 {
	_ = s.loginAsSeller()
	...
	s.NotNil(resp)
	if resp == nil {
		s.T().Skip("Register failed")
		return 0
	}
	s.Equal(float64(0), resp["error"])
	auctionID, ok := resp["auctionId"].(float64)
	if !ok || auctionID <= 0 {
		s.T().Skip("No auction id in register response")
		return 0
	}
	return uint64(auctionID)
}
...
func (s *AuctionTestSuite) TestAuctionRegister() {
	...
	if resp != nil {
		...
		s.NotNil(resp["auctionId"])
	}
}

// TestAuctionBuyout 测试一口价购买(买家 002 购买卖家 001 的拍卖)
func (s *AuctionTestSuite) TestAuctionBuyout() {
	// 1. 卖家 001 发布拍卖
	auctionID := s.registerAuctionAsSeller()
	if auctionID == 0 {
		s.T().Skip("No auction to buyout")
		return
	}
...
// TestAuctionBid 测试竞价(买家 002 竞价卖家 001 的拍卖)
func (s *AuctionTestSuite) TestAuctionBid() {
	// 1. 卖家 001 发布拍卖
	auctionID := s.registerAuctionAsSeller()
	if auctionID == 0 {
		s.T().Skip("No auction to bid")
		return
	}
...
// TestAuctionCancel 测试取消拍卖(卖家 001 取消自己发布的拍卖)
func (s *AuctionTestSuite) TestAuctionCancel() {
	// 1. 卖家 001 发布拍卖
	auctionID := s.registerAuctionAsSeller()
	if auctionID == 0 {
		s.T().Skip("No auction to cancel")
		return
	}
...
func (s *AuctionTestSuite) TestAuctionDetail() {
	...
	auctionID := s.registerAuctionAsSeller()
	if auctionID == 0 {
		s.T().Skip("No auction to detail")
		return
	}

	// 2. 买家 002 查看详情
	_ = s.loginAsBuyer()
	resp, err := s.Client.Get(fmt.Sprintf("/api/v1/auction/detail?auctionId=%d", auctionID))
	s.NoError(err)
	s.NotNil(resp)
...
// TestAuctionBidLowPrice 测试低价竞价(低于当前价被拒绝)
func (s *AuctionTestSuite) TestAuctionBidLowPrice() {
	// 1. 卖家 001 发布起拍价 100 的拍卖
	auctionID := s.registerAuctionAsSeller()
	if auctionID == 0 {
		s.T().Skip("No auction to bid")
		return
