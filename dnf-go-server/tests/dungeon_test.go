...
func (s *DungeonTestSuite) setupAuthenticatedClient(openid string) {
	...
	if !ok || len(characters) == 0 {
		createResp, err := s.Client.Post("/api/v1/character/create", map[string]interface{}{
			"name": fmt.Sprintf("DungeonHero%d", time.Now().UnixNano()%10000),
			"job":  1,
		})
		s.NoError(err)
		s.NotNil(createResp)
...
func (s *DungeonTestSuite) TestCharacterOperations() {
	uniqueID := time.Now().UnixNano()
	s.setupAuthenticatedClient(fmt.Sprintf("test_dungeon_%d", uniqueID))
	// 测试基本角色操作是否正常
