...
#### 在 `guild_systems.proto` 文件中添加了以下消息类型：
- `GuildBingoMap`
- `GuildBingoMapAward`
...
- `GuildApplicant`
- `GuildAttendReward`
- `GuildBingoMapClearPastRewardInfo`
- `GuildBingoMapClearRewardInfo`
- `GuildBingoRewardInfo`
- `GuildApplyList`
- `GuildBossInfo`
- `GuildCreate`
- `GuildDetail`
...
### 5. 编写测试
- 创建了 `batch61_test.go` 文件
- 编写了7个测试组，覆盖所有40个消息类型
- 测试用例包含详细的字段验证
...
### 1. 大部分消息类型已存在
- **问题**: 第61批次的40个消息类型中，有38个已经在proto文件中定义
- **解决方案**: 只需要添加2个缺失的消息类型：`GuildBingoMap`和`GuildBingoMapAward`

### 2. 其他批次测试文件错误
- **问题**: 其他批次的测试文件存在字段错误，导致测试编译失败
- **解决方案**: 专注于第61批次的测试验证，确保第61批次的测试代码正确
...
2. **添加了2个缺失的消息定义**：确保所有消息类型都有完整的定义
