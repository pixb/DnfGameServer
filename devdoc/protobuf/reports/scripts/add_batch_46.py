...
def add_batch_46():
    ...
    # 检查批次是否已存在
    existing_batch = tracker.get_batch_by_name("batch_46")
    
    if existing_batch:
        batch_id = existing_batch.id
        print(f"✅ 批次 batch_46 已存在 (ID: {batch_id})")
    else:
        # 创建批次对象
        batch = Batch(
            id=None,
            ...
            blocker=None,
            notes="迁移用户系统、技能系统、舞台系统、聊天系统等相关的消息类型",
            created_at=None,
            updated_at=None
        )
        ...
        print(f"✅ 添加批次: batch_46 (ID: {batch_id})")
