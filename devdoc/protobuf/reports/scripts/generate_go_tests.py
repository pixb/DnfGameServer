...
class GoTestGenerator:
    ...
    def generate_test_file(self, batch_number: int):
        ...
        print(f"✅ 生成测试文件: {test_file_name}")
        print(f"   包含 {len(messages)} 个消息的测试用例")
...
'''
    ...
    def generate_all_batches(self):
        ...
        print()
        print(f"✅ 共生成 {len(batch_numbers)} 个批次的测试文件")

def main():
    """主函数"""
    import argparse
    
    parser = argparse.ArgumentParser(description='生成 Go 测试用例')
    parser.add_argument('--batch', type=int, help='批次号')
    parser.add_argument('--all', action='store_true', help='生成所有批次的测试')
    ...
    if args.batch:
        ...
        parser.print_help()
        print()
        print("💡 提示: 使用 --batch <批次号> 生成特定批次的测试")
        print("💡 提示: 使用 --all 生成所有批次的测试")
