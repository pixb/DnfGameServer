...
public class TC003_获取角色列表 {
    ...
    public void testGetCharacterList() throws Exception {
        ...
        assertNotNull("角色列表为空", res.charlist);
        assertTrue("角色列表为空", res.charlist.size() > 0);
        System.out.println("角色列表验证通过，角色数量: " + res.charlist.size());

        for (PT_CHARACTER_INFO character : res.charlist) {
