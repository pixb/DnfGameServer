...
public class TC004_加入队伍_无效队伍GUID {
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC004: 加入队伍_无效队伍GUID ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testJoinPartyWithInvalidGuid() throws Exception {
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        System.out.println("\n步骤8: 构造无效队伍GUID加入队伍请求");
        REQ_CONTROL_GROUP reqControlGroup = new REQ_CONTROL_GROUP();
        reqControlGroup.type = 14; // 加入队伍类型
        reqControlGroup.partyguid = -1L; // 无效队伍GUID
        reqControlGroup.isplayer = false;
        System.out.println("REQ_CONTROL_GROUP对象创建成功（包含无效队伍GUID）");

        System.out.println("\n步骤9: 编码无效队伍GUID加入队伍请求");
        ...
        System.out.println("\n步骤10: 发送无效队伍GUID加入队伍请求");
        out.write(reqData);
        out.flush();
        System.out.println("无效队伍GUID加入队伍请求发送成功");

        System.out.println("\n步骤11: 接收无效队伍GUID加入队伍响应");
        ...
        while (!responseReceived && System.currentTimeMillis() - startTime < timeout) {
            try {
                byte[] joinResponseBytes = readMessage(in);
                Object msg = MessageCodec.decodeMessage(joinResponseBytes);
        ...
        System.out.println("\n步骤12: 验证无效队伍GUID加入队伍响应");
        System.out.println("无效队伍GUID加入队伍测试通过");
    }
