...
public class TC008_搜索队伍_无效条件 {
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC008: 搜索队伍_无效条件 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testSearchPartyWithInvalidConditions() throws Exception {
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        System.out.println("REQ_SEARCH_PARTY_LIST对象创建成功（包含无效搜索条件）");

        System.out.println("\n步骤9: 编码无效搜索条件请求");
        byte[] reqData = MessageCodec.encodeMessage(reqSearchPartyList, (byte)(seq + 1));
        ...
        out.write(reqData);
        out.flush();
        System.out.println("无效搜索条件请求发送成功");
        ...
        while (!responseReceived && System.currentTimeMillis() - startTime < timeout) {
            try {
                byte[] searchResponseBytes = readMessage(in);
                Object msg = MessageCodec.decodeMessage(searchResponseBytes);
