...
public class TC015_队伍异常_并发操作 {

    private static final String SERVER_HOST = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final int CONNECT_TIMEOUT = 10000;
    private static final String TEST_OPENID = "test_openid_015";
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC015: 队伍异常_并发操作 ==========");
        prepareTestData();
    }
    ...
    public void tearDown() throws Exception {
        ...
        cleanTestData();
        System.out.println("========== TC015 测试结束 ==========");
    }

    @Test
    public void testConcurrentPartyOperations() throws Exception {
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        System.out.println("\n步骤8: 构造并发操作队伍请求1 - 修改队伍设置");
        REQ_CONTROL_GROUP reqControlGroup1 = new REQ_CONTROL_GROUP();
        reqControlGroup1.type = 18; // 修改队伍设置类型
        reqControlGroup1.partyguid = 39466795435429301L; // 队伍GUID
        reqControlGroup1.minlevel = 5;
        reqControlGroup1.maxlevel = 80;
        reqControlGroup1.partyname = "并发操作测试队伍";
        System.out.println("REQ_CONTROL_GROUP对象1创建成功");

        System.out.println("\n步骤9: 编码并发操作请求1");
        byte[] reqData1 = MessageCodec.encodeMessage(reqControlGroup1, (byte)(seq + 1));
        assertNotNull("编码失败", reqData1);
        assertTrue("编码数据为空", reqData1.length > 0);
        System.out.println("编码成功，数据长度: " + reqData1.length);

        System.out.println("\n步骤10: 发送并发操作请求1");
        out.write(reqData1);
        out.flush();
        System.out.println("并发操作请求1发送成功");

        System.out.println("\n步骤11: 构造并发操作队伍请求2 - 离开队伍");
        REQ_CONTROL_GROUP reqControlGroup2 = new REQ_CONTROL_GROUP();
        reqControlGroup2.type = 16; // 离开队伍类型
        reqControlGroup2.partyguid = 39466795435429301L; // 队伍GUID
        System.out.println("REQ_CONTROL_GROUP对象2创建成功");

        System.out.println("\n步骤12: 编码并发操作请求2");
        byte[] reqData2 = MessageCodec.encodeMessage(reqControlGroup2, (byte)(seq + 2));
        assertNotNull("编码失败", reqData2);
        assertTrue("编码数据为空", reqData2.length > 0);
        System.out.println("编码成功，数据长度: " + reqData2.length);

        System.out.println("\n步骤13: 发送并发操作请求2");
        out.write(reqData2);
        out.flush();
        System.out.println("并发操作请求2发送成功");
        ...
        while (!responseReceived && System.currentTimeMillis() - startTime < timeout) {
            try {
                ...
                Object msg = MessageCodec.decodeMessage(respData);
