...
public class TC002_创建队伍_无效参数 {

    private static final String SERVER_HOST = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final int CONNECT_TIMEOUT = 10000;
    private static final String TEST_OPENID = "test_openid_002";
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC002: 创建队伍_无效参数 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testCreatePartyWithInvalidParams() throws Exception {
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqControlGroup.type = 13; // 创建队伍类型
        reqControlGroup.area = -1; // 无效区域
        reqControlGroup.subtype = -1; // 无效子类型
        reqControlGroup.stageindex = -1; // 无效阶段索引
        reqControlGroup.dungeonindex = -1; // 无效副本索引
        reqControlGroup.minlevel = -1; // 无效最低等级
        reqControlGroup.maxlevel = -1; // 无效最高等级
        reqControlGroup.partyname = "";
        reqControlGroup.isplayer = false;
        System.out.println("REQ_CONTROL_GROUP对象创建成功（包含无效参数）");
        ...
        out.write(reqData);
        out.flush();
        System.out.println("无效参数创建队伍请求发送成功");
        ...
        while (!responseReceived && System.currentTimeMillis() - startTime < timeout) {
            try {
                byte[] createResponseBytes = readMessage(in);
                Object msg = MessageCodec.decodeMessage(createResponseBytes);
        ...
        System.out.println("\n步骤12: 验证无效参数创建队伍响应");
        System.out.println("无效参数创建队伍测试通过");
    }
