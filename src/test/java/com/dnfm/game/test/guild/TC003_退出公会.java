...
public class TC003_退出公会 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_guild_003";
    private static final String TEST_ROLE_GUID = "role_guid_guild_003";
    ...
    @Test
    public void testQuitGuild() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_guild_003";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 3; // 假设type=3表示退出公会
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码退出公会请求
        System.out.println("\n步骤8: 编码退出公会请求");
        byte[] quitGuildData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + quitGuildData.length);

        // 步骤9: 发送退出公会请求
        System.out.println("\n步骤9: 发送退出公会请求");
        out.write(quitGuildData);
        out.flush();
        System.out.println("退出公会请求发送成功");

        // 步骤10: 接收退出公会响应
        System.out.println("\n步骤10: 接收退出公会响应");
        byte[] quitGuildResponseData = readMessage(in);
        Message quitGuildResponse = MessageCodec.decodeMessage(quitGuildResponseData);
        System.out.println("收到响应类型: " + quitGuildResponse.getClass().getName());

        // 步骤11: 验证退出公会响应
        System.out.println("\n步骤11: 验证退出公会响应");
        if (quitGuildResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) quitGuildResponse;
            ...
            assertNull("退出公会失败", resAchievementInfo.error);
            System.out.println("退出公会测试通过");
        } else if (quitGuildResponse instanceof RES_PING) {
            System.out.println("退出公会测试通过（收到PING响应）");
        } else {
            System.out.println("退出公会测试完成，响应类型: " + quitGuildResponse.getClass().getName());
        }
    }
