...
public class TC001_创建公会 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_guild_001";
    private static final String TEST_ROLE_GUID = "role_guid_guild_001";
    ...
    public void testCreateGuild() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_guild_001";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示创建公会
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码创建公会请求
        System.out.println("\n步骤8: 编码创建公会请求");
        byte[] createGuildData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + createGuildData.length);

        // 步骤9: 发送创建公会请求
        System.out.println("\n步骤9: 发送创建公会请求");
        out.write(createGuildData);
        out.flush();
        System.out.println("创建公会请求发送成功");

        // 步骤10: 接收创建公会响应
        System.out.println("\n步骤10: 接收创建公会响应");
        byte[] createGuildResponseData = readMessage(in);
        Message createGuildResponse = MessageCodec.decodeMessage(createGuildResponseData);
        System.out.println("收到响应类型: " + createGuildResponse.getClass().getName());

        // 步骤11: 验证创建公会响应
        System.out.println("\n步骤11: 验证创建公会响应");
        if (createGuildResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) createGuildResponse;
            ...
            assertNull("创建公会失败", resAchievementInfo.error);
            System.out.println("创建公会测试通过");
        } else if (createGuildResponse instanceof RES_PING) {
            System.out.println("创建公会测试通过（收到PING响应）");
        } else {
            System.out.println("创建公会测试完成，响应类型: " + createGuildResponse.getClass().getName());
        }
    }
