...
public class TC002_加入公会 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_guild_002";
    private static final String TEST_ROLE_GUID = "role_guid_guild_002";
    ...
    public void testJoinGuild() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_guild_002";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 2; // 假设type=2表示加入公会
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码加入公会请求
        System.out.println("\n步骤8: 编码加入公会请求");
        byte[] joinGuildData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + joinGuildData.length);

        // 步骤9: 发送加入公会请求
        System.out.println("\n步骤9: 发送加入公会请求");
        out.write(joinGuildData);
        out.flush();
        System.out.println("加入公会请求发送成功");

        // 步骤10: 接收加入公会响应
        System.out.println("\n步骤10: 接收加入公会响应");
        byte[] joinGuildResponseData = readMessage(in);
        Message joinGuildResponse = MessageCodec.decodeMessage(joinGuildResponseData);
        System.out.println("收到响应类型: " + joinGuildResponse.getClass().getName());

        // 步骤11: 验证加入公会响应
        System.out.println("\n步骤11: 验证加入公会响应");
        if (joinGuildResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) joinGuildResponse;
            ...
            assertNull("加入公会失败", resAchievementInfo.error);
            System.out.println("加入公会测试通过");
        } else if (joinGuildResponse instanceof RES_PING) {
            System.out.println("加入公会测试通过（收到PING响应）");
        } else {
            System.out.println("加入公会测试完成，响应类型: " + joinGuildResponse.getClass().getName());
        }
    }
