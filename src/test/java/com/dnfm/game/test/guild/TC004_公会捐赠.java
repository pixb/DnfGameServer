...
public class TC004_公会捐赠 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_guild_004";
    private static final String TEST_ROLE_GUID = "role_guid_guild_004";
    ...
    public void testGuildDonation() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_guild_004";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 4; // 假设type=4表示公会捐赠
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码公会捐赠请求
        System.out.println("\n步骤8: 编码公会捐赠请求");
        byte[] donationData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + donationData.length);

        // 步骤9: 发送公会捐赠请求
        System.out.println("\n步骤9: 发送公会捐赠请求");
        out.write(donationData);
        out.flush();
        System.out.println("公会捐赠请求发送成功");

        // 步骤10: 接收公会捐赠响应
        System.out.println("\n步骤10: 接收公会捐赠响应");
        byte[] donationResponseData = readMessage(in);
        Message donationResponse = MessageCodec.decodeMessage(donationResponseData);
        System.out.println("收到响应类型: " + donationResponse.getClass().getName());

        // 步骤11: 验证公会捐赠响应
        System.out.println("\n步骤11: 验证公会捐赠响应");
        if (donationResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) donationResponse;
        ...
        } else if (donationResponse instanceof RES_PING) {
            System.out.println("公会捐赠测试通过（收到PING响应）");
        } else {
            System.out.println("公会捐赠测试完成，响应类型: " + donationResponse.getClass().getName());
        }
    }
