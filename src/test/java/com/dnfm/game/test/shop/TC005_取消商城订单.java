...
public class TC005_取消商城订单 {
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC005: 取消商城订单 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testCancelShopOrder() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqAchievementInfo.type = 5; // 假设type=5表示取消商城订单
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码取消商城订单请求
        System.out.println("\n步骤8: 编码取消商城订单请求");
        byte[] cancelShopOrderData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + cancelShopOrderData.length);

        // 步骤9: 发送取消商城订单请求
        System.out.println("\n步骤9: 发送取消商城订单请求");
        out.write(cancelShopOrderData);
        out.flush();
        System.out.println("取消商城订单请求发送成功");

        // 步骤10: 接收取消商城订单响应
        System.out.println("\n步骤10: 接收取消商城订单响应");
        byte[] cancelShopOrderResponseData = readMessage(in);
        Message cancelShopOrderResponse = MessageCodec.decodeMessage(cancelShopOrderResponseData);
        System.out.println("收到响应类型: " + cancelShopOrderResponse.getClass().getName());

        // 步骤11: 验证取消商城订单响应
        System.out.println("\n步骤11: 验证取消商城订单响应");
        if (cancelShopOrderResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) cancelShopOrderResponse;
            ...
            assertNull("取消商城订单失败", resAchievementInfo.error);
            System.out.println("取消商城订单测试通过");
        } else if (cancelShopOrderResponse instanceof RES_PING) {
            System.out.println("取消商城订单测试通过（收到PING响应）");
        } else {
            System.out.println("取消商城订单测试完成，响应类型: " + cancelShopOrderResponse.getClass().getName());
        }
    }
