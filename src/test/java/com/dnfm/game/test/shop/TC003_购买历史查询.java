...
public class TC003_购买历史查询 {
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC003: 购买历史查询 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testQueryBuyHistory() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqAchievementInfo.type = 3; // 假设type=3表示购买历史查询
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码购买历史查询请求
        System.out.println("\n步骤8: 编码购买历史查询请求");
        byte[] queryBuyHistoryData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + queryBuyHistoryData.length);

        // 步骤9: 发送购买历史查询请求
        System.out.println("\n步骤9: 发送购买历史查询请求");
        out.write(queryBuyHistoryData);
        out.flush();
        System.out.println("购买历史查询请求发送成功");

        // 步骤10: 接收购买历史查询响应
        System.out.println("\n步骤10: 接收购买历史查询响应");
        byte[] queryBuyHistoryResponseData = readMessage(in);
        Message queryBuyHistoryResponse = MessageCodec.decodeMessage(queryBuyHistoryResponseData);
        System.out.println("收到响应类型: " + queryBuyHistoryResponse.getClass().getName());

        // 步骤11: 验证购买历史查询响应
        System.out.println("\n步骤11: 验证购买历史查询响应");
        if (queryBuyHistoryResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) queryBuyHistoryResponse;
            ...
            assertNull("购买历史查询失败", resAchievementInfo.error);
            System.out.println("购买历史查询测试通过");
        } else if (queryBuyHistoryResponse instanceof RES_PING) {
            System.out.println("购买历史查询测试通过（收到PING响应）");
        } else {
            System.out.println("购买历史查询测试完成，响应类型: " + queryBuyHistoryResponse.getClass().getName());
        }
    }
