...
public class TC002_购买商城商品 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_shop_002";
    private static final String TEST_ROLE_GUID = "role_guid_shop_002";
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC002: 购买商城商品 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testBuyShopItem() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_shop_002";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 2; // 假设type=2表示购买商城商品
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码购买商城商品请求
        System.out.println("\n步骤8: 编码购买商城商品请求");
        byte[] buyShopItemData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + buyShopItemData.length);

        // 步骤9: 发送购买商城商品请求
        System.out.println("\n步骤9: 发送购买商城商品请求");
        out.write(buyShopItemData);
        out.flush();
        System.out.println("购买商城商品请求发送成功");

        // 步骤10: 接收购买商城商品响应
        System.out.println("\n步骤10: 接收购买商城商品响应");
        byte[] buyShopItemResponseData = readMessage(in);
        Message buyShopItemResponse = MessageCodec.decodeMessage(buyShopItemResponseData);
        System.out.println("收到响应类型: " + buyShopItemResponse.getClass().getName());

        // 步骤11: 验证购买商城商品响应
        System.out.println("\n步骤11: 验证购买商城商品响应");
        if (buyShopItemResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) buyShopItemResponse;
            ...
            assertNull("购买商城商品失败", resAchievementInfo.error);
            System.out.println("购买商城商品测试通过");
        } else if (buyShopItemResponse instanceof RES_PING) {
            System.out.println("购买商城商品测试通过（收到PING响应）");
        } else {
            System.out.println("购买商城商品测试完成，响应类型: " + buyShopItemResponse.getClass().getName());
        }
    }
