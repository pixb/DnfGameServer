...
public class TC005_物品分类查询 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_shop_005";
    private static final String TEST_ROLE_GUID = "role_guid_shop_005";
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC005: 物品分类查询 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testQueryItemCategory() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_shop_005";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 5; // 假设type=5表示物品分类查询
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码物品分类查询请求
        System.out.println("\n步骤8: 编码物品分类查询请求");
        byte[] queryItemCategoryData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + queryItemCategoryData.length);

        // 步骤9: 发送物品分类查询请求
        System.out.println("\n步骤9: 发送物品分类查询请求");
        out.write(queryItemCategoryData);
        out.flush();
        System.out.println("物品分类查询请求发送成功");

        // 步骤10: 接收物品分类查询响应
        System.out.println("\n步骤10: 接收物品分类查询响应");
        byte[] queryItemCategoryResponseData = readMessage(in);
        Message queryItemCategoryResponse = MessageCodec.decodeMessage(queryItemCategoryResponseData);
        System.out.println("收到响应类型: " + queryItemCategoryResponse.getClass().getName());

        // 步骤11: 验证物品分类查询响应
        System.out.println("\n步骤11: 验证物品分类查询响应");
        if (queryItemCategoryResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) queryItemCategoryResponse;
            ...
            assertNull("物品分类查询失败", resAchievementInfo.error);
            System.out.println("物品分类查询测试通过");
        } else if (queryItemCategoryResponse instanceof RES_PING) {
            System.out.println("物品分类查询测试通过（收到PING响应）");
        } else {
            System.out.println("物品分类查询测试完成，响应类型: " + queryItemCategoryResponse.getClass().getName());
        }
    }
