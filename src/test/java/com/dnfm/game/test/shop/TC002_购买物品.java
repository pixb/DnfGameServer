...
public class TC002_购买物品 {
    ...
    public void testBuyItem() throws Exception {
        ...
        reqAchievementInfo.type = 2; // 假设type=2表示购买物品
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码购买物品请求
        System.out.println("\n步骤8: 编码购买物品请求");
        byte[] buyItemData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + buyItemData.length);

        // 步骤9: 发送购买物品请求
        System.out.println("\n步骤9: 发送购买物品请求");
        out.write(buyItemData);
        out.flush();
        System.out.println("购买物品请求发送成功");

        // 步骤10: 接收购买物品响应
        System.out.println("\n步骤10: 接收购买物品响应");
        byte[] buyItemResponseData = readMessage(in);
        Message buyItemResponse = MessageCodec.decodeMessage(buyItemResponseData);
        System.out.println("收到响应类型: " + buyItemResponse.getClass().getName());

        // 步骤11: 验证购买物品响应
        System.out.println("\n步骤11: 验证购买物品响应");
        if (buyItemResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) buyItemResponse;
            ...
            assertNull("购买物品失败", resAchievementInfo.error);
            System.out.println("购买物品测试通过");
        } else if (buyItemResponse instanceof RES_PING) {
            System.out.println("购买物品测试通过（收到PING响应）");
        } else {
            System.out.println("购买物品测试完成，响应类型: " + buyItemResponse.getClass().getName());
        }
    }
