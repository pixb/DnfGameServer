...
public class TC004_查询商城订单 {
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC004: 查询商城订单 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testQueryShopOrder() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqAchievementInfo.type = 4; // 假设type=4表示查询商城订单
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码查询商城订单请求
        System.out.println("\n步骤8: 编码查询商城订单请求");
        byte[] queryShopOrderData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + queryShopOrderData.length);

        // 步骤9: 发送查询商城订单请求
        System.out.println("\n步骤9: 发送查询商城订单请求");
        out.write(queryShopOrderData);
        out.flush();
        System.out.println("查询商城订单请求发送成功");

        // 步骤10: 接收查询商城订单响应
        System.out.println("\n步骤10: 接收查询商城订单响应");
        byte[] queryShopOrderResponseData = readMessage(in);
        Message queryShopOrderResponse = MessageCodec.decodeMessage(queryShopOrderResponseData);
        System.out.println("收到响应类型: " + queryShopOrderResponse.getClass().getName());

        // 步骤11: 验证查询商城订单响应
        System.out.println("\n步骤11: 验证查询商城订单响应");
        if (queryShopOrderResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) queryShopOrderResponse;
            ...
            assertNull("查询商城订单失败", resAchievementInfo.error);
            System.out.println("查询商城订单测试通过");
        } else if (queryShopOrderResponse instanceof RES_PING) {
            System.out.println("查询商城订单测试通过（收到PING响应）");
        } else {
            System.out.println("查询商城订单测试完成，响应类型: " + queryShopOrderResponse.getClass().getName());
        }
    }
