...
public class TC001_获取商城物品列表 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_shop_001";
    private static final String TEST_ROLE_GUID = "role_guid_shop_001";
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC001: 获取商城物品列表 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testGetShopItemList() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_shop_001";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示获取商城物品列表
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码获取商城物品列表请求
        System.out.println("\n步骤8: 编码获取商城物品列表请求");
        byte[] getShopItemListData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + getShopItemListData.length);

        // 步骤9: 发送获取商城物品列表请求
        System.out.println("\n步骤9: 发送获取商城物品列表请求");
        out.write(getShopItemListData);
        out.flush();
        System.out.println("获取商城物品列表请求发送成功");

        // 步骤10: 接收获取商城物品列表响应
        System.out.println("\n步骤10: 接收获取商城物品列表响应");
        byte[] getShopItemListResponseData = readMessage(in);
        Message getShopItemListResponse = MessageCodec.decodeMessage(getShopItemListResponseData);
        System.out.println("收到响应类型: " + getShopItemListResponse.getClass().getName());

        // 步骤11: 验证获取商城物品列表响应
        System.out.println("\n步骤11: 验证获取商城物品列表响应");
        if (getShopItemListResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) getShopItemListResponse;
            ...
            assertNull("获取商城物品列表失败", resAchievementInfo.error);
            System.out.println("获取商城物品列表测试通过");
        } else if (getShopItemListResponse instanceof RES_PING) {
            System.out.println("获取商城物品列表测试通过（收到PING响应）");
        } else {
            System.out.println("获取商城物品列表测试完成，响应类型: " + getShopItemListResponse.getClass().getName());
        }
    }
