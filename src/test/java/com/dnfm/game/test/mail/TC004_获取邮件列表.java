...
public class TC004_获取邮件列表 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_mail_004";
    private static final String TEST_ROLE_GUID = "role_guid_mail_004";
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC004: 获取邮件列表 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testGetMailList() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_mail_004";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 4; // 假设type=4表示获取邮件列表
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码获取邮件列表请求
        System.out.println("\n步骤8: 编码获取邮件列表请求");
        byte[] getMailListData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + getMailListData.length);

        // 步骤9: 发送获取邮件列表请求
        System.out.println("\n步骤9: 发送获取邮件列表请求");
        out.write(getMailListData);
        out.flush();
        System.out.println("获取邮件列表请求发送成功");

        // 步骤10: 接收获取邮件列表响应
        System.out.println("\n步骤10: 接收获取邮件列表响应");
        byte[] getMailListResponseData = readMessage(in);
        Message getMailListResponse = MessageCodec.decodeMessage(getMailListResponseData);
        System.out.println("收到响应类型: " + getMailListResponse.getClass().getName());

        // 步骤11: 验证获取邮件列表响应
        System.out.println("\n步骤11: 验证获取邮件列表响应");
        if (getMailListResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) getMailListResponse;
            ...
            assertNull("获取邮件列表失败", resAchievementInfo.error);
            System.out.println("获取邮件列表测试通过");
        } else if (getMailListResponse instanceof RES_PING) {
            System.out.println("获取邮件列表测试通过（收到PING响应）");
        } else {
            System.out.println("获取邮件列表测试完成，响应类型: " + getMailListResponse.getClass().getName());
        }
    }
