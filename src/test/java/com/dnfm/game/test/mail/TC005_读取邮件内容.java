...
public class TC005_读取邮件内容 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_mail_005";
    private static final String TEST_ROLE_GUID = "role_guid_mail_005";
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC005: 读取邮件内容 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testReadMailContent() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_mail_005";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 5; // 假设type=5表示读取邮件内容
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码读取邮件内容请求
        System.out.println("\n步骤8: 编码读取邮件内容请求");
        byte[] readMailContentData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + readMailContentData.length);

        // 步骤9: 发送读取邮件内容请求
        System.out.println("\n步骤9: 发送读取邮件内容请求");
        out.write(readMailContentData);
        out.flush();
        System.out.println("读取邮件内容请求发送成功");

        // 步骤10: 接收读取邮件内容响应
        System.out.println("\n步骤10: 接收读取邮件内容响应");
        byte[] readMailContentResponseData = readMessage(in);
        Message readMailContentResponse = MessageCodec.decodeMessage(readMailContentResponseData);
        System.out.println("收到响应类型: " + readMailContentResponse.getClass().getName());

        // 步骤11: 验证读取邮件内容响应
        System.out.println("\n步骤11: 验证读取邮件内容响应");
        if (readMailContentResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) readMailContentResponse;
            ...
            assertNull("读取邮件内容失败", resAchievementInfo.error);
            System.out.println("读取邮件内容测试通过");
        } else if (readMailContentResponse instanceof RES_PING) {
            System.out.println("读取邮件内容测试通过（收到PING响应）");
        } else {
            System.out.println("读取邮件内容测试完成，响应类型: " + readMailContentResponse.getClass().getName());
        }
    }
