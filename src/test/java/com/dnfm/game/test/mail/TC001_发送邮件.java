...
public class TC001_发送邮件 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_mail_001";
    private static final String TEST_ROLE_GUID = "role_guid_mail_001";
    ...
    public void testSendMail() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_mail_001";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示发送邮件
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码发送邮件请求
        System.out.println("\n步骤8: 编码发送邮件请求");
        byte[] sendMailData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + sendMailData.length);

        // 步骤9: 发送发送邮件请求
        System.out.println("\n步骤9: 发送发送邮件请求");
        out.write(sendMailData);
        out.flush();
        System.out.println("发送邮件请求发送成功");

        // 步骤10: 接收发送邮件响应
        System.out.println("\n步骤10: 接收发送邮件响应");
        byte[] sendMailResponseData = readMessage(in);
        Message sendMailResponse = MessageCodec.decodeMessage(sendMailResponseData);
        System.out.println("收到响应类型: " + sendMailResponse.getClass().getName());

        // 步骤11: 验证发送邮件响应
        System.out.println("\n步骤11: 验证发送邮件响应");
        if (sendMailResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) sendMailResponse;
            ...
            assertNull("发送邮件失败", resAchievementInfo.error);
            System.out.println("发送邮件测试通过");
        } else if (sendMailResponse instanceof RES_PING) {
            System.out.println("发送邮件测试通过（收到PING响应）");
        } else {
            System.out.println("发送邮件测试完成，响应类型: " + sendMailResponse.getClass().getName());
        }
    }
