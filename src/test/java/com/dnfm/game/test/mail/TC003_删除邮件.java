...
public class TC003_删除邮件 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_mail_003";
    private static final String TEST_ROLE_GUID = "role_guid_mail_003";
    ...
    public void testDeleteMail() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_mail_003";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 3; // 假设type=3表示删除邮件
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码删除邮件请求
        System.out.println("\n步骤8: 编码删除邮件请求");
        byte[] deleteMailData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + deleteMailData.length);

        // 步骤9: 发送删除邮件请求
        System.out.println("\n步骤9: 发送删除邮件请求");
        out.write(deleteMailData);
        out.flush();
        System.out.println("删除邮件请求发送成功");

        // 步骤10: 接收删除邮件响应
        System.out.println("\n步骤10: 接收删除邮件响应");
        byte[] deleteMailResponseData = readMessage(in);
        Message deleteMailResponse = MessageCodec.decodeMessage(deleteMailResponseData);
        System.out.println("收到响应类型: " + deleteMailResponse.getClass().getName());

        // 步骤11: 验证删除邮件响应
        System.out.println("\n步骤11: 验证删除邮件响应");
        if (deleteMailResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) deleteMailResponse;
            ...
            assertNull("删除邮件失败", resAchievementInfo.error);
            System.out.println("删除邮件测试通过");
        } else if (deleteMailResponse instanceof RES_PING) {
            System.out.println("删除邮件测试通过（收到PING响应）");
        } else {
            System.out.println("删除邮件测试完成，响应类型: " + deleteMailResponse.getClass().getName());
        }
    }
