...
import com.dnfm.mina.protobuf.generated.PvpRecordInfoRequest;
...
public class TC007_PK排名 {

    private static final String SERVER_HOST = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final int CONNECT_TIMEOUT = 10000;
    private static final String TEST_OPENID = "test_openid_pk_007";
    ...
    @Test
    public void testPvpRanking() throws Exception {
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_pk_007";
        reqLogin.platID = 1;
        ...
        System.out.println("\n步骤8: 构造PK排名请求");
        PvpRecordInfoRequest reqPvpRanking = PvpRecordInfoRequest.newBuilder().build();
        System.out.println("PvpRecordInfoRequest对象创建成功");

        System.out.println("\n步骤9: 编码PK排名请求");
        byte[] pvpRankingEncodedMessage = MessageCodec.encodeMessage(reqPvpRanking, (byte) (seq + 1));
        assertNotNull("编码失败", pvpRankingEncodedMessage);
        assertTrue("编码数据为空", pvpRankingEncodedMessage.length > 0);
        System.out.println("编码成功，数据长度: " + pvpRankingEncodedMessage.length);

        System.out.println("\n步骤10: 发送PK排名请求");
        out.write(pvpRankingEncodedMessage);
        out.flush();
        System.out.println("PK排名请求发送成功");
        ...
        while (!responseReceived && System.currentTimeMillis() - startTime < timeout) {
            try {
                byte[] pvpRankingResponseBytes = readMessage(in);
                Object msg = MessageCodec.decodeMessage(pvpRankingResponseBytes);
