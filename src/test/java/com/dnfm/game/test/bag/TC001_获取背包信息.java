...
public class TC001_获取背包信息 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_bag_001";
    private static final String TEST_ROLE_GUID = "role_guid_bag_001";
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC001: 获取背包信息 ==========");
        prepareTestData();
    }
    ...
    @Test
    public void testGetBagInfo() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_bag_001";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示获取背包信息
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码获取背包信息请求
        System.out.println("\n步骤8: 编码获取背包信息请求");
        byte[] getBagInfoData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + getBagInfoData.length);

        // 步骤9: 发送获取背包信息请求
        System.out.println("\n步骤9: 发送获取背包信息请求");
        out.write(getBagInfoData);
        out.flush();
        System.out.println("获取背包信息请求发送成功");

        // 步骤10: 接收获取背包信息响应
        System.out.println("\n步骤10: 接收获取背包信息响应");
        byte[] getBagInfoResponseData = readMessage(in);
        Message getBagInfoResponse = MessageCodec.decodeMessage(getBagInfoResponseData);
        System.out.println("收到响应类型: " + getBagInfoResponse.getClass().getName());

        // 步骤11: 验证获取背包信息响应
        System.out.println("\n步骤11: 验证获取背包信息响应");
        if (getBagInfoResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) getBagInfoResponse;
            ...
            assertNull("获取背包信息失败", resAchievementInfo.error);
            System.out.println("获取背包信息测试通过");
        } else if (getBagInfoResponse instanceof RES_PING) {
            System.out.println("获取背包信息测试通过（收到PING响应）");
        } else {
            System.out.println("获取背包信息测试完成，响应类型: " + getBagInfoResponse.getClass().getName());
        }
    }
