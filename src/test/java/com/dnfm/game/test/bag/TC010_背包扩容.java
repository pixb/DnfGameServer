...
public class TC010_背包扩容 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_bag_010";
    private static final String TEST_ROLE_GUID = "role_guid_bag_010";
    ...
    @Test
    public void testExpandBag() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_bag_010";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示背包扩容
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码背包扩容请求
        System.out.println("\n步骤8: 编码背包扩容请求");
        byte[] expandBagData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + expandBagData.length);

        // 步骤9: 发送背包扩容请求
        System.out.println("\n步骤9: 发送背包扩容请求");
        out.write(expandBagData);
        out.flush();
        System.out.println("背包扩容请求发送成功");

        // 步骤10: 接收背包扩容响应
        System.out.println("\n步骤10: 接收背包扩容响应");
        byte[] expandBagResponseData = readMessage(in);
        Message expandBagResponse = MessageCodec.decodeMessage(expandBagResponseData);
        System.out.println("收到响应类型: " + expandBagResponse.getClass().getName());

        // 步骤11: 验证背包扩容响应
        System.out.println("\n步骤11: 验证背包扩容响应");
        if (expandBagResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) expandBagResponse;
            ...
            assertNull("背包扩容失败", resAchievementInfo.error);
            System.out.println("背包扩容测试通过");
        } else if (expandBagResponse instanceof RES_PING) {
            System.out.println("背包扩容测试通过（收到PING响应）");
        } else {
            System.out.println("背包扩容测试完成，响应类型: " + expandBagResponse.getClass().getName());
        }
    }
