...
public class TC002_使用物品 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_bag_002";
    private static final String TEST_ROLE_GUID = "role_guid_bag_002";
    ...
    public void testUseItem() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_bag_002";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示使用物品
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码使用物品请求
        System.out.println("\n步骤8: 编码使用物品请求");
        byte[] useItemData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + useItemData.length);

        // 步骤9: 发送使用物品请求
        System.out.println("\n步骤9: 发送使用物品请求");
        out.write(useItemData);
        out.flush();
        System.out.println("使用物品请求发送成功");

        // 步骤10: 接收使用物品响应
        System.out.println("\n步骤10: 接收使用物品响应");
        byte[] useItemResponseData = readMessage(in);
        Message useItemResponse = MessageCodec.decodeMessage(useItemResponseData);
        System.out.println("收到响应类型: " + useItemResponse.getClass().getName());

        // 步骤11: 验证使用物品响应
        System.out.println("\n步骤11: 验证使用物品响应");
        if (useItemResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) useItemResponse;
            ...
            assertNull("使用物品失败", resAchievementInfo.error);
            System.out.println("使用物品测试通过");
        } else if (useItemResponse instanceof RES_PING) {
            System.out.println("使用物品测试通过（收到PING响应）");
        } else {
            System.out.println("使用物品测试完成，响应类型: " + useItemResponse.getClass().getName());
        }
    }
