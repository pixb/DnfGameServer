...
public class TC009_物品重命名 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_bag_009";
    private static final String TEST_ROLE_GUID = "role_guid_bag_009";
    ...
    public void setUp() throws Exception {
        System.out.println("========== TC009: 物品重命名 ==========");
        prepareTestData();
    }
    ...
    public void testRenameItem() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_bag_009";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示物品重命名
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码物品重命名请求
        System.out.println("\n步骤8: 编码物品重命名请求");
        byte[] renameItemData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + renameItemData.length);

        // 步骤9: 发送物品重命名请求
        System.out.println("\n步骤9: 发送物品重命名请求");
        out.write(renameItemData);
        out.flush();
        System.out.println("物品重命名请求发送成功");

        // 步骤10: 接收物品重命名响应
        System.out.println("\n步骤10: 接收物品重命名响应");
        byte[] renameItemResponseData = readMessage(in);
        Message renameItemResponse = MessageCodec.decodeMessage(renameItemResponseData);
        System.out.println("收到响应类型: " + renameItemResponse.getClass().getName());

        // 步骤11: 验证物品重命名响应
        System.out.println("\n步骤11: 验证物品重命名响应");
        if (renameItemResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) renameItemResponse;
            ...
            assertNull("物品重命名失败", resAchievementInfo.error);
            System.out.println("物品重命名测试通过");
        } else if (renameItemResponse instanceof RES_PING) {
            System.out.println("物品重命名测试通过（收到PING响应）");
        } else {
            System.out.println("物品重命名测试完成，响应类型: " + renameItemResponse.getClass().getName());
        }
    }
