...
public class TC003_丢弃物品 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_bag_003";
    private static final String TEST_ROLE_GUID = "role_guid_bag_003";
    ...
    public void testDropItem() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_bag_003";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示丢弃物品
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码丢弃物品请求
        System.out.println("\n步骤8: 编码丢弃物品请求");
        byte[] dropItemData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + dropItemData.length);

        // 步骤9: 发送丢弃物品请求
        System.out.println("\n步骤9: 发送丢弃物品请求");
        out.write(dropItemData);
        out.flush();
        System.out.println("丢弃物品请求发送成功");

        // 步骤10: 接收丢弃物品响应
        System.out.println("\n步骤10: 接收丢弃物品响应");
        byte[] dropItemResponseData = readMessage(in);
        Message dropItemResponse = MessageCodec.decodeMessage(dropItemResponseData);
        System.out.println("收到响应类型: " + dropItemResponse.getClass().getName());

        // 步骤11: 验证丢弃物品响应
        System.out.println("\n步骤11: 验证丢弃物品响应");
        if (dropItemResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) dropItemResponse;
            ...
            assertNull("丢弃物品失败", resAchievementInfo.error);
            System.out.println("丢弃物品测试通过");
        } else if (dropItemResponse instanceof RES_PING) {
            System.out.println("丢弃物品测试通过（收到PING响应）");
        } else {
            System.out.println("丢弃物品测试完成，响应类型: " + dropItemResponse.getClass().getName());
        }
    }
