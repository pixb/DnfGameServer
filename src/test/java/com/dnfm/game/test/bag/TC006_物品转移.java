...
public class TC006_物品转移 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_bag_006";
    private static final String TEST_ROLE_GUID = "role_guid_bag_006";
    ...
    public void testTransferItem() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_bag_006";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示物品转移
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码物品转移请求
        System.out.println("\n步骤8: 编码物品转移请求");
        byte[] transferItemData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + transferItemData.length);

        // 步骤9: 发送物品转移请求
        System.out.println("\n步骤9: 发送物品转移请求");
        out.write(transferItemData);
        out.flush();
        System.out.println("物品转移请求发送成功");

        // 步骤10: 接收物品转移响应
        System.out.println("\n步骤10: 接收物品转移响应");
        byte[] transferItemResponseData = readMessage(in);
        Message transferItemResponse = MessageCodec.decodeMessage(transferItemResponseData);
        System.out.println("收到响应类型: " + transferItemResponse.getClass().getName());

        // 步骤11: 验证物品转移响应
        System.out.println("\n步骤11: 验证物品转移响应");
        if (transferItemResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) transferItemResponse;
            ...
            assertNull("物品转移失败", resAchievementInfo.error);
            System.out.println("物品转移测试通过");
        } else if (transferItemResponse instanceof RES_PING) {
            System.out.println("物品转移测试通过（收到PING响应）");
        } else {
            System.out.println("物品转移测试完成，响应类型: " + transferItemResponse.getClass().getName());
        }
    }
