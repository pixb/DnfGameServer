...
public class TC004_物品合成 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_bag_004";
    private static final String TEST_ROLE_GUID = "role_guid_bag_004";
    ...
    @Test
    public void testCombineItem() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_bag_004";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示物品合成
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码物品合成请求
        System.out.println("\n步骤8: 编码物品合成请求");
        byte[] combineItemData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + combineItemData.length);

        // 步骤9: 发送物品合成请求
        System.out.println("\n步骤9: 发送物品合成请求");
        out.write(combineItemData);
        out.flush();
        System.out.println("物品合成请求发送成功");

        // 步骤10: 接收物品合成响应
        System.out.println("\n步骤10: 接收物品合成响应");
        byte[] combineItemResponseData = readMessage(in);
        Message combineItemResponse = MessageCodec.decodeMessage(combineItemResponseData);
        System.out.println("收到响应类型: " + combineItemResponse.getClass().getName());

        // 步骤11: 验证物品合成响应
        System.out.println("\n步骤11: 验证物品合成响应");
        if (combineItemResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) combineItemResponse;
            ...
            assertNull("物品合成失败", resAchievementInfo.error);
            System.out.println("物品合成测试通过");
        } else if (combineItemResponse instanceof RES_PING) {
            System.out.println("物品合成测试通过（收到PING响应）");
        } else {
            System.out.println("物品合成测试完成，响应类型: " + combineItemResponse.getClass().getName());
        }
    }
