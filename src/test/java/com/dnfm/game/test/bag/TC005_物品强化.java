...
public class TC005_物品强化 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_bag_005";
    private static final String TEST_ROLE_GUID = "role_guid_bag_005";
    ...
    @Test
    public void testEnhanceItem() throws Exception {
        // 步骤1: 建立TCP连接
        System.out.println("\n步骤1: 建立TCP连接");
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_bag_005";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示物品强化
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码物品强化请求
        System.out.println("\n步骤8: 编码物品强化请求");
        byte[] enhanceItemData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + enhanceItemData.length);

        // 步骤9: 发送物品强化请求
        System.out.println("\n步骤9: 发送物品强化请求");
        out.write(enhanceItemData);
        out.flush();
        System.out.println("物品强化请求发送成功");

        // 步骤10: 接收物品强化响应
        System.out.println("\n步骤10: 接收物品强化响应");
        byte[] enhanceItemResponseData = readMessage(in);
        Message enhanceItemResponse = MessageCodec.decodeMessage(enhanceItemResponseData);
        System.out.println("收到响应类型: " + enhanceItemResponse.getClass().getName());

        // 步骤11: 验证物品强化响应
        System.out.println("\n步骤11: 验证物品强化响应");
        if (enhanceItemResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) enhanceItemResponse;
            ...
            assertNull("物品强化失败", resAchievementInfo.error);
            System.out.println("物品强化测试通过");
        } else if (enhanceItemResponse instanceof RES_PING) {
            System.out.println("物品强化测试通过（收到PING响应）");
        } else {
            System.out.println("物品强化测试完成，响应类型: " + enhanceItemResponse.getClass().getName());
        }
    }
