...
public class TC007_物品整理 {

    private static final String SERVER_IP = "127.0.0.1";
    private static final int SERVER_PORT = 10001;
    private static final String TEST_OPENID = "test_openid_bag_007";
    private static final String TEST_ROLE_GUID = "role_guid_bag_007";
    ...
    public void testSortItem() throws Exception {
        ...
        reqLogin.type = 1;
        reqLogin.token = "test_token_bag_007";
        reqLogin.platID = 1;
        ...
        reqAchievementInfo.type = 1; // 假设type=1表示物品整理
        System.out.println("REQ_ACHIEVEMENT_INFO对象创建成功");

        // 步骤8: 编码物品整理请求
        System.out.println("\n步骤8: 编码物品整理请求");
        byte[] sortItemData = MessageCodec.encodeMessage(reqAchievementInfo, (byte) 2);
        System.out.println("编码成功，数据长度: " + sortItemData.length);

        // 步骤9: 发送物品整理请求
        System.out.println("\n步骤9: 发送物品整理请求");
        out.write(sortItemData);
        out.flush();
        System.out.println("物品整理请求发送成功");

        // 步骤10: 接收物品整理响应
        System.out.println("\n步骤10: 接收物品整理响应");
        byte[] sortItemResponseData = readMessage(in);
        Message sortItemResponse = MessageCodec.decodeMessage(sortItemResponseData);
        System.out.println("收到响应类型: " + sortItemResponse.getClass().getName());

        // 步骤11: 验证物品整理响应
        System.out.println("\n步骤11: 验证物品整理响应");
        if (sortItemResponse instanceof RES_ACHIEVEMENT_INFO) {
            RES_ACHIEVEMENT_INFO resAchievementInfo = (RES_ACHIEVEMENT_INFO) sortItemResponse;
            ...
            assertNull("物品整理失败", resAchievementInfo.error);
            System.out.println("物品整理测试通过");
        } else if (sortItemResponse instanceof RES_PING) {
            System.out.println("物品整理测试通过（收到PING响应）");
        } else {
            System.out.println("物品整理测试完成，响应类型: " + sortItemResponse.getClass().getName());
        }
    }
