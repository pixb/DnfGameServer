...
public class AccountMailBox {
   private static final Logger log = LoggerFactory.getLogger(AccountMailBox.class);
   ...
   public void addMail(String title, String msg, List<MailItem> mailItemList) {
      if (!Util.isEmpty(mailItemList)) {
         ...
         log.error("addMail: [{}]", mail.guid);
         this.allMails.put(mail.guid, mail);
      }
   }

   public void removeMail(long guid) {
      this.allMails.remove(guid);
   }
