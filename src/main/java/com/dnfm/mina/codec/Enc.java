...
public class Enc {
   ...
   public static byte[] decrypt2(byte seq, byte[] in) {
      ...
      try {
         byte[] res = lz4Decompress2(scale, out1, in.length);
         return res;
      } catch (Exception e) {
         e.printStackTrace();
         byte[] res = lz4Decompress2(100, out1, in.length);
         return res;
      }
   }
   ...
   public static byte[] lz4Decompress2(int scale, byte[] in, int compressedLen) {
      byte[] restoredData = new byte[compressedLen * scale];
