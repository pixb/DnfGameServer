package com.dnfm.mina.protobuf;

import com.baidu.bjf.remoting.protobuf.Codec;
import com.baidu.bjf.remoting.protobuf.FieldType;
import com.baidu.bjf.remoting.protobuf.ProtobufProxy;
import com.baidu.bjf.remoting.protobuf.code.CodedConstant;
import com.google.protobuf.CodedInputStream;
import com.google.protobuf.CodedOutputStream;
import com.google.protobuf.Descriptors;
import com.google.protobuf.InvalidProtocolBufferException;
import java.io.ByteArrayOutputStream;
import java.io.File;
import java.io.IOException;
import java.io.Serializable;
import java.util.ArrayList;
import java.util.List;

public class PT_GACHA_RESULT$$JProtoBufClass implements Codec<PT_GACHA_RESULT>, Serializable {
   public static final long serialVersionUID = 1L;
   private Descriptors.Descriptor descriptor;

   public byte[] encode(PT_GACHA_RESULT var1) throws IOException {
      ByteArrayOutputStream var2 = new ByteArrayOutputStream();
      CodedOutputStream var3 = CodedOutputStream.newInstance(var2);
      this.doWriteTo(var1, var3);
      var3.flush();
      return var2.toByteArray();
   }

   public PT_GACHA_RESULT decode(byte[] var1) throws IOException {
      CodedInputStream var2 = CodedInputStream.newInstance(var1, 0, var1.length);
      return this.readFrom(var2);
   }

   public int size(PT_GACHA_RESULT var1) throws IOException {
      int var2 = 0;
      Object var3 = null;
      if (!CodedConstant.isNull(var1.recipe)) {
         Integer var18 = var1.recipe;
         var2 += CodedOutputStream.computeInt32Size(1, var18);
      }

      Object var4 = null;
      if (!CodedConstant.isNull(var1.gachaindex)) {
         Integer var19 = var1.gachaindex;
         var2 += CodedOutputStream.computeInt32Size(2, var19);
      }

      Object var5 = null;
      if (!CodedConstant.isNull(var1.equip)) {
         List var20 = var1.equip;
         var2 += CodedConstant.computeListSize(3, var20, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var6 = null;
      if (!CodedConstant.isNull(var1.material)) {
         List var21 = var1.material;
         var2 += CodedConstant.computeListSize(4, var21, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var7 = null;
      if (!CodedConstant.isNull(var1.avatar)) {
         List var22 = var1.avatar;
         var2 += CodedConstant.computeListSize(5, var22, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var8 = null;
      if (!CodedConstant.isNull(var1.emblem)) {
         List var23 = var1.emblem;
         var2 += CodedConstant.computeListSize(6, var23, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var9 = null;
      if (!CodedConstant.isNull(var1.card)) {
         List var24 = var1.card;
         var2 += CodedConstant.computeListSize(7, var24, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var10 = null;
      if (!CodedConstant.isNull(var1.creature)) {
         List var25 = var1.creature;
         var2 += CodedConstant.computeListSize(8, var25, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var11 = null;
      if (!CodedConstant.isNull(var1.cartifact)) {
         List var26 = var1.cartifact;
         var2 += CodedConstant.computeListSize(9, var26, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var12 = null;
      if (!CodedConstant.isNull(var1.consume)) {
         List var27 = var1.consume;
         var2 += CodedConstant.computeListSize(10, var27, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var13 = null;
      if (!CodedConstant.isNull(var1.crack)) {
         List var28 = var1.crack;
         var2 += CodedConstant.computeListSize(11, var28, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var14 = null;
      if (!CodedConstant.isNull(var1.crackequip)) {
         List var29 = var1.crackequip;
         var2 += CodedConstant.computeListSize(12, var29, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var15 = null;
      if (!CodedConstant.isNull(var1.sdavatar)) {
         List var30 = var1.sdavatar;
         var2 += CodedConstant.computeListSize(13, var30, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var16 = null;
      if (!CodedConstant.isNull(var1.bookmark)) {
         List var31 = var1.bookmark;
         var2 += CodedConstant.computeListSize(14, var31, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      Object var17 = null;
      if (!CodedConstant.isNull(var1.scroll)) {
         List var32 = var1.scroll;
         var2 += CodedConstant.computeListSize(15, var32, FieldType.OBJECT, false, (File)ProtobufProxy.OUTPUT_PATH.get(), false);
      }

      return var2;
   }

   public void doWriteTo(PT_GACHA_RESULT var1, CodedOutputStream var2) throws IOException {
      Object var3 = null;
      if (!CodedConstant.isNull(var1.recipe)) {
         Integer var18 = var1.recipe;
         if (var18 != null) {
            var2.writeInt32(1, var18);
         }
      }

      Object var4 = null;
      if (!CodedConstant.isNull(var1.gachaindex)) {
         Integer var19 = var1.gachaindex;
         if (var19 != null) {
            var2.writeInt32(2, var19);
         }
      }

      Object var5 = null;
      if (!CodedConstant.isNull(var1.equip)) {
         List var20 = var1.equip;
         if (var20 != null) {
            CodedConstant.writeToList(var2, 3, FieldType.OBJECT, var20, false);
         }
      }

      Object var6 = null;
      if (!CodedConstant.isNull(var1.material)) {
         List var21 = var1.material;
         if (var21 != null) {
            CodedConstant.writeToList(var2, 4, FieldType.OBJECT, var21, false);
         }
      }

      Object var7 = null;
      if (!CodedConstant.isNull(var1.avatar)) {
         List var22 = var1.avatar;
         if (var22 != null) {
            CodedConstant.writeToList(var2, 5, FieldType.OBJECT, var22, false);
         }
      }

      Object var8 = null;
      if (!CodedConstant.isNull(var1.emblem)) {
         List var23 = var1.emblem;
         if (var23 != null) {
            CodedConstant.writeToList(var2, 6, FieldType.OBJECT, var23, false);
         }
      }

      Object var9 = null;
      if (!CodedConstant.isNull(var1.card)) {
         List var24 = var1.card;
         if (var24 != null) {
            CodedConstant.writeToList(var2, 7, FieldType.OBJECT, var24, false);
         }
      }

      Object var10 = null;
      if (!CodedConstant.isNull(var1.creature)) {
         List var25 = var1.creature;
         if (var25 != null) {
            CodedConstant.writeToList(var2, 8, FieldType.OBJECT, var25, false);
         }
      }

      Object var11 = null;
      if (!CodedConstant.isNull(var1.cartifact)) {
         List var26 = var1.cartifact;
         if (var26 != null) {
            CodedConstant.writeToList(var2, 9, FieldType.OBJECT, var26, false);
         }
      }

      Object var12 = null;
      if (!CodedConstant.isNull(var1.consume)) {
         List var27 = var1.consume;
         if (var27 != null) {
            CodedConstant.writeToList(var2, 10, FieldType.OBJECT, var27, false);
         }
      }

      Object var13 = null;
      if (!CodedConstant.isNull(var1.crack)) {
         List var28 = var1.crack;
         if (var28 != null) {
            CodedConstant.writeToList(var2, 11, FieldType.OBJECT, var28, false);
         }
      }

      Object var14 = null;
      if (!CodedConstant.isNull(var1.crackequip)) {
         List var29 = var1.crackequip;
         if (var29 != null) {
            CodedConstant.writeToList(var2, 12, FieldType.OBJECT, var29, false);
         }
      }

      Object var15 = null;
      if (!CodedConstant.isNull(var1.sdavatar)) {
         List var30 = var1.sdavatar;
         if (var30 != null) {
            CodedConstant.writeToList(var2, 13, FieldType.OBJECT, var30, false);
         }
      }

      Object var16 = null;
      if (!CodedConstant.isNull(var1.bookmark)) {
         List var31 = var1.bookmark;
         if (var31 != null) {
            CodedConstant.writeToList(var2, 14, FieldType.OBJECT, var31, false);
         }
      }

      Object var17 = null;
      if (!CodedConstant.isNull(var1.scroll)) {
         List var32 = var1.scroll;
         if (var32 != null) {
            CodedConstant.writeToList(var2, 15, FieldType.OBJECT, var32, false);
         }
      }

   }

   public void writeTo(PT_GACHA_RESULT var1, CodedOutputStream var2) throws IOException {
      this.doWriteTo(var1, var2);
   }

   public PT_GACHA_RESULT readFrom(CodedInputStream var1) throws IOException {
      PT_GACHA_RESULT var2 = new PT_GACHA_RESULT();
      var2.equip = new ArrayList();
      var2.material = new ArrayList();
      var2.avatar = new ArrayList();
      var2.emblem = new ArrayList();
      var2.card = new ArrayList();
      var2.creature = new ArrayList();
      var2.cartifact = new ArrayList();
      var2.consume = new ArrayList();
      var2.crack = new ArrayList();
      var2.crackequip = new ArrayList();
      var2.sdavatar = new ArrayList();
      var2.bookmark = new ArrayList();
      var2.scroll = new ArrayList();

      try {
         boolean var3 = false;
         Object var4 = null;

         while(!var3) {
            int var5 = var1.readTag();
            if (var5 == 0) {
               break;
            }

            if (var5 == 8) {
               var2.recipe = var1.readInt32();
            } else if (var5 == 16) {
               var2.gachaindex = var1.readInt32();
            } else if (var5 == 26) {
               Codec var10 = ProtobufProxy.create(PT_EQUIP.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var6 = var1.readRawVarint32();
               int var7 = var1.pushLimit(var6);
               if (var2.equip == null) {
                  var2.equip = new ArrayList();
               }

               var2.equip.add((PT_EQUIP)var10.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var7);
            } else if (var5 == 34) {
               Codec var11 = ProtobufProxy.create(PT_STACKABLE.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var23 = var1.readRawVarint32();
               int var35 = var1.pushLimit(var23);
               if (var2.material == null) {
                  var2.material = new ArrayList();
               }

               var2.material.add((PT_STACKABLE)var11.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var35);
            } else if (var5 == 42) {
               Codec var12 = ProtobufProxy.create(PT_AVATAR_ITEM.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var24 = var1.readRawVarint32();
               int var36 = var1.pushLimit(var24);
               if (var2.avatar == null) {
                  var2.avatar = new ArrayList();
               }

               var2.avatar.add((PT_AVATAR_ITEM)var12.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var36);
            } else if (var5 == 50) {
               Codec var13 = ProtobufProxy.create(PT_STACKABLE.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var25 = var1.readRawVarint32();
               int var37 = var1.pushLimit(var25);
               if (var2.emblem == null) {
                  var2.emblem = new ArrayList();
               }

               var2.emblem.add((PT_STACKABLE)var13.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var37);
            } else if (var5 == 58) {
               Codec var14 = ProtobufProxy.create(PT_STACKABLE.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var26 = var1.readRawVarint32();
               int var38 = var1.pushLimit(var26);
               if (var2.card == null) {
                  var2.card = new ArrayList();
               }

               var2.card.add((PT_STACKABLE)var14.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var38);
            } else if (var5 == 66) {
               Codec var15 = ProtobufProxy.create(PT_CREATURE.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var27 = var1.readRawVarint32();
               int var39 = var1.pushLimit(var27);
               if (var2.creature == null) {
                  var2.creature = new ArrayList();
               }

               var2.creature.add((PT_CREATURE)var15.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var39);
            } else if (var5 == 74) {
               Codec var16 = ProtobufProxy.create(PT_ARTIFACT.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var28 = var1.readRawVarint32();
               int var40 = var1.pushLimit(var28);
               if (var2.cartifact == null) {
                  var2.cartifact = new ArrayList();
               }

               var2.cartifact.add((PT_ARTIFACT)var16.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var40);
            } else if (var5 == 82) {
               Codec var17 = ProtobufProxy.create(PT_STACKABLE.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var29 = var1.readRawVarint32();
               int var41 = var1.pushLimit(var29);
               if (var2.consume == null) {
                  var2.consume = new ArrayList();
               }

               var2.consume.add((PT_STACKABLE)var17.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var41);
            } else if (var5 == 90) {
               Codec var18 = ProtobufProxy.create(PT_STACKABLE.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var30 = var1.readRawVarint32();
               int var42 = var1.pushLimit(var30);
               if (var2.crack == null) {
                  var2.crack = new ArrayList();
               }

               var2.crack.add((PT_STACKABLE)var18.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var42);
            } else if (var5 == 98) {
               Codec var19 = ProtobufProxy.create(PT_EQUIP.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var31 = var1.readRawVarint32();
               int var43 = var1.pushLimit(var31);
               if (var2.crackequip == null) {
                  var2.crackequip = new ArrayList();
               }

               var2.crackequip.add((PT_EQUIP)var19.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var43);
            } else if (var5 == 106) {
               Codec var20 = ProtobufProxy.create(PT_AVATAR_ITEM.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var32 = var1.readRawVarint32();
               int var44 = var1.pushLimit(var32);
               if (var2.sdavatar == null) {
                  var2.sdavatar = new ArrayList();
               }

               var2.sdavatar.add((PT_AVATAR_ITEM)var20.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var44);
            } else if (var5 == 114) {
               Codec var21 = ProtobufProxy.create(PT_STACKABLE.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var33 = var1.readRawVarint32();
               int var45 = var1.pushLimit(var33);
               if (var2.bookmark == null) {
                  var2.bookmark = new ArrayList();
               }

               var2.bookmark.add((PT_STACKABLE)var21.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var45);
            } else if (var5 == 122) {
               Codec var22 = ProtobufProxy.create(PT_EQUIP.class, false, (File)ProtobufProxy.OUTPUT_PATH.get());
               int var34 = var1.readRawVarint32();
               int var46 = var1.pushLimit(var34);
               if (var2.scroll == null) {
                  var2.scroll = new ArrayList();
               }

               var2.scroll.add((PT_EQUIP)var22.readFrom(var1));
               var1.checkLastTagWas(0);
               var1.popLimit(var46);
            } else {
               var1.skipField(var5);
            }
         }

         return var2;
      } catch (InvalidProtocolBufferException var8) {
         throw var8;
      } catch (IOException var9) {
         throw var9;
      }
   }

   public Descriptors.Descriptor getDescriptor() throws IOException {
      if (this.descriptor != null) {
         return this.descriptor;
      } else {
         Descriptors.Descriptor var1 = CodedConstant.getDescriptor(PT_GACHA_RESULT.class);
         return this.descriptor = var1;
      }
   }
}
