...
def main():
    ...
    # 如果指定了批次文件，从批次文件读取
    if args.batch:
        print(f'使用批次文件: {args.batch}')
        with open(args.batch, 'r', encoding='utf-8') as f:
            batch_class_names = [line.strip() for line in f if line.strip()]
        
        # 查找对应的 .class 文件
        class_files = []
        for class_name in batch_class_names:
            class_file = Path(args.class_dir) / class_name
            if class_file.exists():
                class_files.append(class_file)
            else:
                print(f'警告: 文件不存在: {class_file}')
        
        print(f'从批次文件中找到 {len(class_files)} 个文件')
    else:
        # 遍历所有 .class 文件
        class_files = list(Path(args.class_dir).rglob('*.class'))
        print(f'找到 {len(class_files)} 个 .class 文件')
    ...
    print(f'找到 {len(message_classes)} 个消息类')
    ...
    for class_file in message_classes:
        ...
        if is_enum_type(javap_output):
            ...
            # 调试信息
            if enum_values:
                print(f'  找到 {len(enum_values)} 个枚举值在 {class_name}')
            else:
                print(f'  没有找到枚举值在 {class_name}')
            ...
            proto_path = os.path.join(args.proto_dir, proto_filename)
            
            generate_enum_proto_file(class_name, enum_values, proto_path)
            generated_count += 1
            print(f'已生成: {proto_path}')
            continue
        ...
        # 调试信息
        if fields:
            print(f'  找到 {len(fields)} 个字段在 {class_name}')
        else:
            print(f'  没有找到字段在 {class_name}')
    ...
    print(f'\n总共生成了 {generated_count} 个 proto 文件')
